...
class SongsByPlaylistScreen extends StatefulWidget {
   SongsByPlaylistScreen({super.key,required this.playlistname, required this.playlistindex,required this.allPlaylistSongs});
...
plstsongs.add(Audio.file(song.songurl,metas: Metas(title: song.songname,artist: song.artist,id: song.id.toString())));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBar(widget.playlistname, IconButton(onPressed: (){Navigator.pop(context);}, icon: const Icon(Icons.arrow_back_ios_new_rounded),splashRadius: 25,),),
      body: ValueListenableBuilder<Box<PlaylistSongs>>(
        ...
        builder: (context,value,_){
          ...
          if(songs!.isEmpty){
            return  Center(child: Text('No Songs Added !',style: textWhite18,),);
          }
            ...
            itemBuilder: (context, index) {
              return ListTile(
                ...
                player.open(Playlist(audios: plstsongs, startIndex: index),
                          ...
                          Navigator.of(context).push(MaterialPageRoute(builder: (context)=> MusicPlayerScreen(index: index)));
                },
                leading: const Image(
